...
const Sidebar: React.FC<SidebarProps> = () => {
    ...
    const fetchLikedSongs = useCallback(async () => {
        if (!user?._id) return;
        try {
            // You might want to add this endpoint to fetch liked songs count
            const res = await axios.get<{ count: number }>(
                "http://localhost:3000/user/liked-songs/count",
                { withCredentials: true }
            );
            // Handle count if endpoint exists
        } catch (error) {
    ...
    useEffect(() => {
        ...
        setRecentSongs(recent.slice(0, 5)); // Limit to 5 recent songs
    }, []);
    ...
    const NavItem = ({
        ...
        >
            <Icon size={20} className={`${active ? 'text-emerald-400 drop-shadow-lg' : 'text-gray-300 group-hover:text-white'} transition-colors`} />
            <span className={`font-medium text-sm transition-all ${isExpanded ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-2 pointer-events-none'}`}>
                {label}
            </span>
            {badge !== undefined && (
            ...
            <div className={`absolute left-0 top-2 bottom-2 w-1 bg-gradient-to-b from-emerald-400 to-teal-500 rounded-full opacity-0 group-hover:opacity-100 transition-all ${active ? 'opacity-100' : ''}`} />
        </Link>
    );

    return (
        <div className={`bg-gradient-to-b from-gray-900/95 via-black/50 to-gray-900/80 backdrop-blur-xl h-screen flex flex-col transition-all duration-300 overflow-hidden border-r border-white/10 shadow-2xl shadow-black/50 ${isExpanded ? 'w-72' : 'w-20'}`}>
            ...
            <div className="p-4 border-b border-white/10 sticky top-0 bg-black/30 backdrop-blur-sm z-10">
                <div className="flex items-center gap-3">
                    ...
                    <div className={`flex flex-col overflow-hidden transition-all ${isExpanded ? 'w-48' : 'w-0'}`}>
            ...
            <div className="p-3 space-y-1 flex-1 overflow-y-auto">
                <NavItem icon={Home} label="Home" active={window.location.pathname === '/'} to="/" />


                {user && (
                    <>
                        <div className={`border-t border-white/10 pt-4 mt-4 ${isExpanded ? 'block' : 'hidden'}`}>
                            <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider px-1 mb-3">
                                Playlists
                            </h3>
                        </div>
                    </>
            ...
            {/* User Content */}
            {user && (
                <div className="p-3 space-y-3 border-t border-white/10 sticky bottom-0 bg-black/30 backdrop-blur-sm z-10">
                    ...
                    <div className={`${isExpanded ? 'block' : 'hidden'}`}>
                        ...
                        {playlists.length === 0 ? (
                            ...
                            <div className="space-y-1.5 max-h-32 overflow-y-auto custom-scrollbar">
                                ...
                                {playlists.length > 4 && (
                                    <div className={`text-xs text-gray-500 p-2 rounded-xl bg-white/5 ${isExpanded ? 'block' : 'hidden'}`}>
                                ...
                                recentSongs.map((song) => (
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <p className="text-sm font-medium text-white truncate group-hover:text-emerald-300 transition-colors">{song.title}</p>
                                            <p className="text-xs text-gray-400 truncate">{song.artist}</p>
                                        </div>
                                        <Play size={14} className="text-emerald-400 opacity-0 group-hover:opacity-100 transition-opacity ml-auto" />
                                    </div>
                                ))
                            )}
                        </div>
                ...
                <div className="p-4 border-t border-white/10 sticky bottom-0 bg-black/30 backdrop-blur-sm z-10">
                    <div className="text-center space-y-2">
                        ...
                        <p className="text-sm text-gray-400">
                            <Link to="/signin" className="text-emerald-400 font-medium hover:underline hover:text-emerald-300 transition-colors">
                                Sign in
                            </Link>{' '}
